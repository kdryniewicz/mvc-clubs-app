namespace Rad301ClubsV1.Migrations.ClubModelMigrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Rad301ClubsV1.Models.ClubModel.ClubContext>
    {
        ...
        public void seedCourses(Rad301ClubsV1.Models.ClubModel.ClubContext context)
        {
            Assembly assembly = Assembly.GetExecutingAssembly();
            ...
            using (Stream stream = assembly.GetManifestResourceStream(resourceName))
            {
                using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
                {
                    ...
                    csvReader.Configuration.HasHeaderRecord = false;
                    csvReader.Configuration.WillThrowOnMissingField = false;
                    var testCourses = csvReader.GetRecords<Course>().ToArray();
                    context.Courses.AddOrUpdate(c => c.ID, testCourses);
                }
            }
        }
    }
